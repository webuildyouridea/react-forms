...
var Form = function (_React$Component) {
    ...
    _this._validateInput = function (textInput) {
      ...
      try {
        for (var _iterator = validators[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          ...
          var isPristine = textInput.state.isPristine;
          var value = textInput.props.value;
      ...
      textInput.setState({ isValid: isValid });
      return isValid;
    };
  ...
  _createClass(Form, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      ...
      try {
        for (var _iterator2 = nextProps.forceErrorOn[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          var error = _step2.value;

          var fieldName = Object.keys(error)[0];

          var fieldComponent = this._inputs[fieldName];
          fieldComponent.setState({
            isValid: false,
            errors: _defineProperty({}, fieldName, error[fieldName])
          });
        }
      } catch (err) {
    ...
    value: function _getFieldObjectFromInput(textInput) {
      ...
      var _textInput$state = textInput.state,
          ...
          isValid = _textInput$state.isValid,
          isPristine = _textInput$state.isPristine,
          _textInput$state$erro = _textInput$state.errors,
          errors = _textInput$state$erro === undefined ? null : _textInput$state$erro;
